...
Текущая тестовая версия: `0.44.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.44.0`:

- готовые режимы клиента: без ограничений, ребёнок, гость и IoT;
- скорость выбирается в Мбит/с, а рядом показывается фактически измеренный текущий трафик;
- расписание, DNS-фильтр, приоритет и лимиты применяются одним проверяемым действием;
- блокировка интернета и закрепление текущего IPv4 доступны прямо в карточке клиента;
- Web UI и Android используют единый каталог режимов и скоростей с сервера.
