...
Текущая тестовая версия: `0.41.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.41.0`:

- клиент сохраняет одну запись при смене IP-адреса или интерфейса, пока его MAC не меняется;
- Web UI, Android и API показывают одинаковый тип устройства и источник его определения;
- карточка клиента получила историю состояний `online`, `recent` и `offline`;
- отдельные API детальной карточки, истории присутствия и приращений трафика пригодны для мобильного интерфейса;
- ручной выбор типа устройства не перезаписывается последующей telemetry.
