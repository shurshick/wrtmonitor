...
Текущая тестовая версия: `0.53.0 Release Candidate`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.53.0`:

- общий профиль доступа можно назначить клиенту, основной или гостевой Wi-Fi-сети;
- профиль задаёт блокировку, расписание и суммарные лимиты приёма/передачи для SSID;
- Web UI и Android позволяют создавать, редактировать, назначать и снимать профили;
- агент передаёт фактически применённый профиль и строго проверяет расписание и traffic control.
