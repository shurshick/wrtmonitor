...
Текущая стабильная версия: `v0.16.0-stability`.

Главное в `0.16.0`:

- команды получили единый версионированный контракт, idempotency и проверку результата на роутере;
- Android автоматически восстанавливает истёкшую сессию без повторного входа;
- миграции PostgreSQL защищены от параллельного запуска, добавлены readiness и метрики;
- обновления агента проверяются Ed25519-подписью, а ключ агента можно безопасно сменить;
- контейнер запускается без root, зависимости проверяются аудитом уязвимостей.
