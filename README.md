...
Текущая тестовая версия: `0.38.1`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.38.1`:

- аппаратный контракт проверен на Netis NX31 и OpenWrt x86;
- агент сохраняет все реальные thermal trip points, а не только warning/critical;
- Netis показывает SoC и оба Wi-Fi-датчика, x86 без датчика честно возвращает `unsupported`;
- неизвестная плата автоматически получает наблюдаемый профиль по точной идентичности;
- аппаратный JSON-отчёт можно скачать из Web UI или через API для пополнения каталога.
