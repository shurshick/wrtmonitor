...
Текущая тестовая версия: `v0.23.0-maintenance`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.23.0`:

- пакеты apk/opkg устанавливаются, обновляются и удаляются отдельными командами;
- резервные копии скачиваются и проверяются перед восстановлением;
- sysupgrade проходит preflight по HTTPS, SHA-256, модели и совместимости;
- Web UI показывает процессы, init-службы и текущее расписание cron;
- диагностический архив и recovery mode доступны из служебного раздела.
