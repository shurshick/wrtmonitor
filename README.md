...
Текущая стабильная версия: `v0.10.1`.

Главное в `0.10.1`:

- исправлено обновление агента `0.9.0 -> 0.10.x`;
- stable и RC версии сравниваются численно;
- защита от настоящего downgrade сохранена;
- сервер автоматически применяет compatibility-режим только для проблемного перехода.
