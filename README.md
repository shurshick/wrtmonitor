...
Текущая стабильная версия: `v0.13.7`.

Главное в `0.13.7`:

- обзор и Android показывают число действительно активных клиентов, а не размер истории DHCP;
- Wi-Fi эфир выводит только полученные от драйвера сигнал, скорость, длительность подключения и airtime;
- трафик клиентов появляется только при работающем nlbwmon и наличии реальных счётчиков;
- сохранённые и отключённые устройства остаются в реестре, но не искажают оперативную telemetry.
