...
Текущая тестовая версия: `v0.31.1-hardware-certification`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.31.1`:

- все 91 команды прогнаны на Netis NX31 и OpenWrt x86: 173 проверки прошли, 9 корректно отмечены как неподдерживаемые;
- VPN policy сохраняет фактический UCI section и не падает из-за отключённой службы PBR;
- повторное отключение отсутствующего Wi-Fi mesh стало безопасной идемпотентной операцией;
- обновление агента поддерживает Ed25519 и совместимую со старым OpenSSL подпись RSA/SHA-256;
- установщик завершает оставшиеся процессы старого агента и очищает блокировки перед установкой.
...
```text
wrtmonitor-truenas-v0.31.1.yaml
```
