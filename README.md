...
Текущая тестовая версия: `v0.30.0-openwrt-modules`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.30.0`:

- агент обнаруживает USB-накопители, принтеры и LTE/USB-модемы;
- SMB, NFS, FTP и DLNA доступны как управляемые модули;
- зависимости каждого модуля устанавливаются агентом автоматически;
- Web UI и Android показывают только поддерживаемые или уже установленные компоненты;
- результат установки и состояние службы проверяются до завершения команды.
...
```text
wrtmonitor-truenas-v0.30.0.yaml
```
