...
Текущая тестовая версия: `0.38.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.38.0`:

- сервер автоматически создаёт наблюдаемый профиль неизвестной платы по точной Device Tree идентичности, не угадывая модель по общему SoC;
- проверенный каталог содержит точные профили Netis NX31, Banana Pi BPI-R3, Xiaomi AX3600 и NanoPi R5S;
- дублирующие датчики одного аппаратного узла нормализуются, а реальные пределы берутся только из `thermal`/`hwmon` sysfs;
- Web UI и Android получили отдельный раздел «Оборудование» с происхождением профиля, CPU, температурами и троттлингом;
- состояния `normal`, `warning` и `critical` вычисляются только при наличии фактического порога; иначе честно показывается «порог неизвестен».
