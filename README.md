...
Текущая стабильная версия: `v0.13.6`.

Главное в `0.13.6`:

- агент поддерживает `apk` в OpenWrt 25.12+ и сохраняет совместимость с `opkg`;
- пакеты, обновления и операции установки показываются через фактический менеджер роутера;
- резервные копии доступны без ложной проверки отдельной команды `gzip`;
- системные пути агента включают `/sbin` и `/usr/sbin`, поэтому `sysupgrade` определяется в фоне корректно.
