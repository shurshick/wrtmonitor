...
Текущая тестовая версия: `0.55.4 Android Overview Polish`. Публичного стабильного релиза пока нет. Версионированные базы обновляются миграциями без пересоздания; границы совместимости и порядок восстановления описаны в [правилах обновления](docs/database-upgrades.md).

Главное в `0.55.4`:

- плитки ресурсов роутера в Android получили одинаковую высоту;
- быстрые действия собраны в ровную сетку из шести кнопок;
- добавлен быстрый переход к состоянию системы;
- короткие подписи больше не обрезаются на телефоне;
- сервер, API, база данных и команды OpenWrt не менялись. Подробности в [описании релиза](RELEASE_NOTES.md).
