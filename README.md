...
Текущая тестовая версия: `v0.20.0-internet-network`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.20.0`:

- WAN работает в режимах DHCP, static и PPPoE без лишних полей;
- IPv6, DNS, DoT/DoH, bridge, VLAN и сегменты используют реальные UCI-данные;
- порты показывают speed, duplex, carrier, traffic и ошибки;
- Multi-WAN получил health-check, failover и возврат на основной канал;
- Web UI и Android используют один контракт сетевых настроек.
