...
- обязательные зависимости агента устанавливаются и проверяются при установке и каждом обновлении;
- `nlbwmon` настраивается, включается, запускается и автоматически восстанавливается при сбое запроса;
- физические порты показывают скорость, duplex, трафик, пакеты и ошибки;
- WAN-форма показывает только параметры выбранного протокола;
- Web UI и Android управляют DNS over TLS и DNS over HTTPS с установкой пакета одной кнопкой.
