...
Текущая тестовая версия: `v0.30.1-agent-stability`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.30.1`:

- изменение LAN не перезапускает всю сеть и не рвёт рабочее соединение при совпадающей конфигурации;
- post-condition проверяет фактический IPv4-адрес интерфейса и не сообщает ложный успех;
- зависшие init-сервисы ограничены по времени и больше не блокируют цикл агента;
- незавершённые конфигурационные транзакции восстанавливаются после перезапуска;
- секретные поля удаляются из telemetry до записи в PostgreSQL.
...
```text
wrtmonitor-truenas-v0.30.1.yaml
```
