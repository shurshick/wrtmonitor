...
Текущая тестовая версия: `v0.19.0-command-reliability`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.19.0`:

- все 80 команд описывают гарантии доставки и проверки результата;
- повторная доставка одного command UUID не выполняет изменение второй раз;
- OpenWrt harness проверяет UCI, ubus, firewall и сервисы;
- ошибки содержат конкретную причину, код и возможность повтора;
- аппаратный отчёт не принимает `PASS` без доказательства.
