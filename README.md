...
Текущая тестовая версия: `0.42.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.42.0`:

- карточка клиента и политика сохраняются одной атомарной операцией без частично применённого состояния;
- Android загружает детальную карточку, 50 событий присутствия и 96 интервалов клиентского трафика;
- Web UI и Android позволяют удалить сохранённую запись клиента вместе с локальной историей;
- активное устройство после удаления может быть обнаружено заново, что явно объясняется перед действием;
- старые `PUT/PATCH` и `apply-policy` сохранены для совместимости API, основной путь — `configure`.
