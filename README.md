...
Текущая тестовая версия: `v0.32.0-push-reliability`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.32.0`:

- агент получает команды через long-poll без ожидания следующей telemetry;
- Web UI и Android обновляют открытый экран через единый SSE-поток;
- интервал telemetry остаётся независимым и настраиваемым от 5 секунд;
- при потере сервера агент использует ограниченный экспоненциальный backoff;
- добавлены метрики подключений, пробуждений, timeout и переполнения событий.
...
```text
wrtmonitor-truenas-v0.32.0.yaml
```
