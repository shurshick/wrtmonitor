...
Текущая тестовая версия: `v0.24.0-full-parity`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.24.0`:

- Android использует тот же актуальный набор операций управления, что и Web UI;
- NAT, WireGuard и OpenVPN управляются по фактическим конфигурационным записям;
- экраны явно различают загрузку, устаревшие данные, ошибку и неподдерживаемую функцию;
- выбранный роутер и раздел восстанавливаются после пересоздания Android Activity;
- обслуживание в Android включает пакеты, процессы, cron и init-службы.
