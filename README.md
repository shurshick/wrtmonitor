...
Текущая тестовая версия: `v0.34.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.34.0`:

- брокер реального времени переведён с In-Memory на PostgreSQL `LISTEN/NOTIFY` для мульти-инстанс работы;
- добавлен эндпоинт `/metrics` в формате Prometheus;
- агент сохраняет оффлайн-буфер телеметрии при падении сети и аккуратно досылает данные при её восстановлении.
...
```text
wrtmonitor-truenas-v0.34.0.yaml
```
