...
Текущая тестовая версия: `0.50.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.50.0`:

- экран «О приложении» в Android собран заново: компактная структура, версия, автор и описание больше не слипаются;
- скачивание APK и переход на страницу GitHub разделены на два честных действия;
- кнопка обновления прямо сообщает, что скачивает новую версию;
- повторные загрузки APK больше не конфликтуют с файлом предыдущего обновления;
- выбор обновления закрыт регрессионными Android-тестами.
