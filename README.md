...
Текущая тестовая версия: `0.40.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.40.0`:

- на главной собраны состояние сети, ресурсы и только действительно важные события;
- перезагрузка, Wi-Fi, гостевая сеть, блокировка клиента и диагностика доступны как быстрые действия;
- Web UI и Android используют одинаковые команды, состояния и сообщения;
- существующая гостевая сеть включается без повторного ввода сохранённого пароля;
- технические коды событий заменены понятными пользователю названиями.
