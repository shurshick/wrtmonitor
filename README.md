...
Текущая тестовая версия: `v0.24.2-ui-polish`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.24.2`:

- тема Web UI сохраняется при переходах между разделами;
- компактные карточки обслуживания занимают свободное место рядом с мониторингом;
- перезагрузка доступна компактной иконкой в списках Web UI и Android;
- системная нагрузка объяснена относительно числа ядер и исходного `load average`.
