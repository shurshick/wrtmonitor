...
Текущая тестовая версия: `0.45.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.45.0`:

- Web UI и Android показывают одинаковое фактическое состояние радиомодулей, сетей и доступных каналов;
- состояние, канал, регион, режим и мощность выбранного радиомодуля сохраняются одной операцией;
- подключение к Wi-Fi по QR не требует ручного ввода пароля: ключ читается агентом из OpenWrt;
- Wi-Fi URI выдаётся один раз и не сохраняется в PostgreSQL, кэше агента или журнале операций;
- роутер без физического Wi-Fi получает честное состояние `unsupported` без пустых форм управления.
