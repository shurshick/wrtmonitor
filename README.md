...
Текущая тестовая версия: `0.35.14`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в серии `0.35.x`:

- добавлены групповые политики (Fleet Management);
- реализован Web SSH терминал прямо из браузера через WebSocket туннель без проброса портов;
