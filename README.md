...
Текущая тестовая версия: `0.52.0 Release Candidate`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.52.0`:

- диагностический архив и метрики агентов формируются без роста числа запросов к PostgreSQL на каждый роутер;
- повторное нажатие отправки отзыва не создаёт дубликат;
- частота отправки отзывов ограничена, а Web UI и API возвращают понятное состояние;
- новые функции в Release Candidate не добавляются: релиз посвящён надёжности и производительности.
