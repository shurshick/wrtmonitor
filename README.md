...
Текущая стабильная версия: `v0.13.4`.

Главное в `0.13.4`:

- Android получил плотный список клиентов и отдельный экран выбранного устройства;
- список группируется по SSID, проводному подключению и доступности;
- недоступные устройства свёрнуты, IPv4 больше не теряется за IPv6;
- агент передаёт диапазон, SSID и параметры активных Wi-Fi-клиентов;
- сервер отличает реально активных клиентов от старых DHCP-аренд.

В предыдущем релизе `0.13.3`:

- постоянный IPv4 закрепляется прямо в параметрах выбранного клиента;
- правила firewall, зоны и транзит редактируются по реальным записям OpenWrt;
- пакеты opkg и резервные копии получили рабочие элементы управления.

В предыдущем релизе `0.13.2`:

- клиенты домашней сети стали компактным списком вместо крупных плиток;
- добавлены поиск, фильтры и раскрываемые параметры доступа.

В предыдущем релизе `0.13.1`:

- диапазоны telemetry в Android загружаются без гонки запросов и показывают фактически загруженный период;
- Web UI и Android получили шкалы времени и значений, легенду и графики трафика, нагрузки, памяти и клиентов;
- Android-разделы «Система» и «Обслуживание» разделены по задачам, как в Web UI;
- browser E2E проверяет реальное переключение периода и метрики, а не только наличие canvas.

В предыдущем релизе `0.13.0`:
