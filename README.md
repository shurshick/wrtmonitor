...
Текущая тестовая версия: `0.52.4 Release Candidate`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.52.4`:

- пакет обновляется после подтверждения без перезагрузки всей страницы и закрытия списка;
- официальный каталог находит новую прошивку по ветке OpenWrt и реальному профилю платы;
- Netis NX31 корректно сопоставляется с профилем `netis_nx31` и видит OpenWrt 25.12.5;
- Android не отправляет неполную конфигурацию новой гостевой сети.
