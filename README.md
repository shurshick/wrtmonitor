...
Текущая тестовая версия: `v0.28.0-operations-and-notifications`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.28.0`:

- исправлен сбор клиентского трафика через `nlbwmon`;
- добавлены уведомления и метрики эксплуатации;
- очистка telemetry и операций выполняется планово;
- диагностический архив сервера скачивается через API;
- новые образы проходят canary-выдержку перед переводом в `latest`.
...
```text
wrtmonitor-truenas-v0.28.0.yaml
```
