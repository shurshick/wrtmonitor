...
Текущая тестовая версия: `0.52.2 Release Candidate`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.52.2`:

- после обновления отдельного пакета агент сразу перечитывает каталог и отправляет свежую telemetry;
- счётчик доступных обновлений уменьшается без ожидания обычного интервала обмена;
- `success` возвращается только если обновлённый пакет действительно исчез из списка доступных обновлений;
- обновление каталога также немедленно синхронизирует Web UI и Android.
