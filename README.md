...
- Web UI сгруппирован по задачам: состояние, подключения, домашняя сеть, безопасность и управление;
- сетевые правила и VPN больше не смешаны с настройкой обычного интернет-подключения;
- Android получил пять основных вкладок и отдельный каталог дополнительных функций;
- переходы между разделами Android работают иерархически и не сбрасывают выбранный роутер;
- функциональность по-прежнему определяется реальными capabilities конкретного OpenWrt.

В предыдущем релизе `0.10.4`:

- Web UI корректно определяет HTTPS за reverse proxy;
- локальный HTTP показывает причину блокировки защищённой cookie;
- логин нормализуется одинаково в Web UI и Android API.
