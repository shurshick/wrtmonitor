...
Текущая тестовая версия: `v0.22.0-firewall-vpn`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.22.0`:

- firewall rules, zones, forwarding, NAT и статические маршруты изменяются по фактическим UCI section;
- WireGuard, OpenVPN и PBR показывают настроенное и runtime-состояние раздельно;
- профили OpenVPN включаются, отключаются, экспортируются и удаляются из списка;
- VPN и сетевые изменения автоматически откатываются, если агент теряет связь с сервером;
- ошибки и post-condition проходят через общий контракт надёжности команд.
