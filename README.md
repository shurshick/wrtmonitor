...
Текущая тестовая версия: `0.46.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.46.0`:

- Android API разделён на транспорт, авторизацию, операции и JSON-преобразования без изменения публичного контракта;
- VPN и аппаратные карточки вынесены из крупных Compose-экранов;
- Web CSS разделён на основу, компоненты и адаптивные правила с сохранением порядка каскада;
- CI фиксирует новые пределы размера для Android API, ключевых экранов и стилей.

Главное в `0.45.2`:

- агент устанавливает новый runtime целым поколением и переключается на него только после полной проверки;
- отказ записи, нехватка места или неполный payload останавливают обновление до замены рабочего entrypoint;
- rollback восстанавливает согласованный комплект entrypoint, init-скрипта и библиотек;
- повторная установка по существующему token сначала получает настоящий `device_id` с сервера;
- installer проверяет доступность файловой системы до установки пакетов и остановки службы.
