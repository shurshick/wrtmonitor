...
Текущая тестовая версия: `0.49.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.49.0`:

- полный контракт из 94 команд повторно проверен на Netis NX31 и OpenWrt x86;
- reboot подтверждается сменой `boot_id` и новой telemetry, а не старым статусом `online`;
- update, rollback и отключение агента получили обязательную runtime-проверку;
- Web SSH проверяется через настоящий браузер: ввод, resize, reconnect и close;
- старый cache key terminal JavaScript удалён.
