...
Текущая стабильная версия: `v0.14.2`.

Главное в `0.14.2`:

- `nlbwmon` запускается после установки, а его счётчики разбираются по именам колонок;
- поиск по установленным пакетам работает сразу при вводе названия;
- реальные настройки IPv6 LAN, RA, DHCPv6 и NDP доступны в Web UI и Android;
- IPv6 домашней сети больше не смешан с настройками WAN.
