...
Текущая тестовая версия: `0.55.3 Release Reliability`. Публичного стабильного релиза пока нет. Версионированные базы обновляются миграциями без пересоздания; границы совместимости и порядок восстановления описаны в [правилах обновления](docs/database-upgrades.md).

Главное в `0.55.3`:

- исправлено применение профилей доступа к отдельным клиентам и Wi-Fi;
- исправлены стили и штатное завершение Web SSH;
- зафиксирована исходная схема БД, описаны миграции и восстановление;
- усилены проверки безопасности и публикации контейнерного `latest`;
- выпуск без нового длительного аппаратного прогона разрешён владельцем; автоматические проверки остаются обязательными. Подробности в [описании релиза](RELEASE_NOTES.md).
