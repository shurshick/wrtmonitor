...
Текущая тестовая версия: `0.36.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.36.0`:

- появился постоянный журнал событий с подтверждением, отложенным просмотром и сроком хранения;
- правила уведомлений поддерживают Web UI, Android, webhook, ntfy и серверный SMTP;
- безопасные сценарии запускают только типизированные команды с cooldown, лимитом и dry-run;
- результат автоматизации связан с фактическим ответом OpenWrt-агента;
- опасные команды и циклы `command -> command` запрещены.
