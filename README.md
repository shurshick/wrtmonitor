...
- dashboard обновляется без перезагрузки и адаптируется под телефон и компьютер;
- Android показывает тот же live traffic monitor и ресурсную сводку;
- история telemetry доступна через защищённый API владельца;
- браузерный E2E проверяет фактическую отрисовку графика.
