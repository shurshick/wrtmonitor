...
Текущая стабильная версия: `v0.10.4`.

Главное в `0.10.4`:

- Web UI корректно определяет HTTPS за reverse proxy;
- локальный HTTP больше не создаёт ложное впечатление успешного входа;
- интерфейс объясняет блокировку защищённой cookie и ведёт на публичный HTTPS-адрес;
- логин нормализуется одинаково в Web UI и Android API.

В предыдущем релизе `0.10.3`:

- журнал операций показывает пять записей на страницу;
- завершённые команды хранятся 30 дней и ограничены 500 записями на роутер;
- активные команды не попадают под очистку;
- срок хранения и лимит настраиваются через переменные окружения.

В предыдущем релизе `0.10.2`:
...
- сохранена совместимость с прежним форматом telemetry.
