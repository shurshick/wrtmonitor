...
Текущая тестовая версия: `0.47.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.47.0`:

- SQLAlchemy-модели разделены по предметным областям без изменения таблиц PostgreSQL;
- Web-страница роутера и маршруты команд разбиты на компактные HTTP-обработчики и сборщики контекста;
- идентичность и присутствие клиентов отделены от инвентаризации;
- аппаратные профили, сопоставление наблюдений и отчёты больше не смешаны в одном сервисе;
- CI блокирует повторное разрастание архитектурных фасадов.
