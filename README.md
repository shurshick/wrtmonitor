...
Текущая тестовая версия: `v0.18.0-truth-layer`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.18.0`:

- API больше не подменяет отсутствующие наблюдения нулями и пустыми объектами;
- состояние данных единообразно описывается как `configured`, `observed`, `unsupported`, `stale` или `error`;
- telemetry принимает только явную актуальную схему v2;
- после управляющей команды агент отправляет свежее наблюдение до terminal result;
- PostgreSQL сведён к одной исходной схеме для чистой тестовой установки.

- восстановлено автоматическое обновление агентов `0.15.x` до актуальной версии;
- detached-подпись манифеста проверяется отдельно и больше не ломает legacy checksum-проверку;
- добавлен постоянный регрессионный тест совместимости старого updater с новыми release payload;
- функциональность сегментов сети и VLAN из `0.17.0` сохранена без изменений;
- подписанный Android APK обновлён до `versionCode 63`.

Основной функционал `0.17.0`:

- сегменты LAN, Guest и IoT создаются и изменяются из Web UI и Android;
- физические порты назначаются реальным OpenWrt bridge без дублирования UCI-конфигурации;
- добавлено управление Bridge VLAN 802.1Q с tagged/untagged портами;
- перед применением проверяются пересечения подсетей, DHCP-пулы, занятые порты и дубликаты VLAN;
- каждое изменение защищено UCI backup, проверкой восстановления связи и автоматическим rollback.
