...
Текущая тестовая версия: `0.45.1`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.45.1`:

- состояние, фактическая работа и влияние расписания Wi-Fi больше не смешиваются;
- Web UI и Android читают расписание, гостевую сеть и Mesh выбранного радиомодуля из одной telemetry;
- переключение диапазона подставляет только его реальные канал, режим, регион, мощность и настройки сети;
- пустой пароль гостевой сети или Mesh сохраняет существующий ключ, а не стирает его;
- Netis NX31 подтверждает два работающих радиомодуля; устройство без Wi-Fi получает `unsupported`.
