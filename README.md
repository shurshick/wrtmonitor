...
Текущая тестовая версия: `0.51.0 Public Beta`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.51.0`:

- диагностика сети запускается одной кнопкой в Web и Android;
- безопасный отчёт роутера можно скачать в Web или передать из Android;
- архив диагностики сервера включает состояние всех подключённых роутеров;
- обратная связь отправляется из Web UI и экрана «О приложении» Android;
- отчёты и отзывы не содержат пароли, токены и сырую конфигурацию роутера.
