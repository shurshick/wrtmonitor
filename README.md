...
Текущая тестовая версия: `v0.29.0-network-policy`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.29.0`:

- добавлены реальные профили SQM, расписание и клиентские DNS-политики;
- журнал Multi-WAN фиксирует фактические смены основного канала;
- формы Firewall и VPN получили готовые безопасные шаблоны;
- каталог прошивок строится по официальному `profiles.json` OpenWrt и модели роутера;
- ручной URL прошивки оставлен только как аварийный режим.
...
```text
wrtmonitor-truenas-v0.29.0.yaml
```
