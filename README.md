...
Текущая тестовая версия: `v0.27.0-operational-parity`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.27.0`:

- матрица CI подтверждает доступность всех 90 операций в Web UI, Android, API и агенте;
- варианты интерфейсов, сетей, зон, радиомодулей и каналов приходят с роутера;
- Android-экраны управления работают через Repository и сохраняют выбранное состояние;
- Android UI-тесты запускаются на эмуляторе и блокируют выпуск при поломке навигации;
- ошибки и подтверждения опасных действий используют единый серверный контракт.
...
```text
wrtmonitor-truenas-v0.27.0.yaml
```
