...
Текущая тестовая версия: `0.35.17`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в серии `0.35.x`:

- добавлены групповые политики (Fleet Management);
- Web-терминал использует локальный xterm, отдельную PTY-сессию и PostgreSQL-брокер без проброса портов роутера;
- терминальный канал работает независимо от периода telemetry и подтверждает запуск только после готовности PTY и обоих HTTPS-направлений;
- внедрена криптографическая проверка подписей Ed25519 и dual-RSA trust при автоматических обновлениях агента;
- исправлены PostgreSQL realtime, SSH-авторизация и групповые команды.
