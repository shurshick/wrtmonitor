...
Текущая тестовая версия: `v0.21.0-wifi-clients`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.21.0`:

- диапазоны 2.4/5/6 ГГц, SSID, guest, isolation, roaming, mesh и расписания используют единый контракт;
- survey радиомодуля передаёт наблюдаемые загрузку эфира, шум и время занятости либо явную причину недоступности;
- Wi-Fi-клиенты показывают signal, noise, RX/TX bitrate, airtime и длительность подключения без вымышленных значений;
- `nlbwmon` получил самодиагностику и автоматическую попытку восстановления;
- Android редактирует сеть целиком: сегмент, защиту, скрытие, изоляцию и 802.11r/k/v.
