...
Текущая тестовая версия: `0.55.2 Web Control Center`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.
...
- безопасные изменения показывают предварительную проверку и отправку команды;
- Web SSH больше не дублирует заголовок и аккуратно помещается в рабочую область экрана;
- Web SSH сохраняет контрастный светлый текст на тёмном терминальном фоне при любой теме интерфейса;
