...
Текущая тестовая версия: `0.52.3 Release Candidate`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.52.3`:

- отдельный пакет в APK теперь действительно обновляется командой `apk upgrade`, а не повторной установкой;
- результат операции содержит установленную версию до и после обновления и целевую версию;
- после подтверждённого обновления список и счётчик доступных пакетов синхронизируются немедленно;
- при отказе журнал показывает пакет и ожидаемую версию вместо общей ошибки post-condition.
