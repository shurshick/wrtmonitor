...
Текущая тестовая версия: `0.48.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.48.0`:

- обновление агента разделено на версию, подписи, хранилище поколения и проверку payload;
- сетевые команды, telemetry и проверка post-condition разделены по подсистемам;
- terminal PTY, DNS/Wi-Fi runtime и транзакционное восстановление больше не смешаны в общих модулях;
- результаты команд имеют единые поля `ok`, `code`, `message` и диагностическую детализацию;
- CI ограничивает размер каждого OpenWrt-модуля и проверяет полный порядок загрузки runtime.
