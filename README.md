...
- Android подключается к уже развёрнутому серверу одноразовым QR-кодом без передачи пароля;
- QR живёт 10 минут, используется один раз и может быть отозван владельцем;
- сервер хранит только hash pairing token и создаёт обычную управляемую access/refresh-сессию;
- в Web UI появились создание QR, обратный отсчёт и управление мобильными сессиями.
