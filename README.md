...
Текущая тестовая версия: `v0.31.2-dns-recovery`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.31.2`:

- установка `https-dns-proxy` и `stubby` больше не оставляет `dnsmasq` на неработающих локальных DNS-портах;
- агент восстанавливает DNS как из собственной резервной копии, так и из метаданных пакета `https-dns-proxy`;
- команды DoH/DoT завершаются успехом только после реального DNS-запроса;
- исправление проверено через серверный API на Netis NX31 и OpenWrt x86.
...
```text
wrtmonitor-truenas-v0.31.2.yaml
```
