...
Текущая тестовая версия: `0.50.1`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.50.1`:

- обновление старого серверного образа проверяется с сохранением PostgreSQL-данных;
- чистая установка, backup verification и restore drill входят в обязательный CI;
- Android восстанавливает поток событий после потери сети и сохраняет зашифрованную сессию;
- Web SSH проверяется при разрыве и повторном подключении;
- опубликован проверяемый семидневный отчёт Netis NX31 и OpenWrt x86;
- тестовые теги больше не перезаписывают `latest` до приёмки.
