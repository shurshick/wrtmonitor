...
Текущая стабильная версия: `v0.16.1-agent-recovery`.

Главное в `0.16.1`:

- повторная установка всегда заменяет старую привязку роутера актуальными `device_id` и token;
- installer проверяет первую telemetry до объявления установки успешной;
- сервер немедленно ставит обновление старого агента в очередь, если автообновление включено;
- после обновления по команде daemon сразу переходит на новую версию.
