...
Текущая тестовая версия: `0.39.0`. Публичного стабильного релиза пока нет; тестовые базы можно пересоздавать между несовместимыми версиями.

Главное в `0.39.0`:

- история температуры, нагрузки, памяти и заполнения накопителя хранится 45 дней;
- WAN, DNS, Wi-Fi, агент и ресурсы сведены в единый экран состояния;
- предупреждения создаются только по наблюдаемым данным и аппаратным thermal-порогам;
- Web UI и Android используют один health-контракт и одинаковые состояния;
- восстановление нормального состояния автоматически закрывает активное предупреждение.
