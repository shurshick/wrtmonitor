...
Версия `0.13.7` использует capability schema 13, поддерживает `apk` и `opkg` и передаёт SSID, диапазон, интерфейс и доступные метрики активных Wi-Fi-клиентов. После обновления сервера агент обновляется обычной кнопкой; переустановка не нужна.
...
Команды `v0.13.7` выполняются только через авторизованный сервер и показываются в интерфейсах по реальным capabilities роутера:
