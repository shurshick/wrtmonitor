...
Версия `0.13.1` сохраняет capability schema 10 и передачу фактического LAN IPv4/префикса/маски, DHCP-пула, часового пояса и NTP. После обновления сервера агент обновляется обычной кнопкой; переустановка не нужна.
...
wget -O wrtmonitor-agent.tar.gz \
  https://github.com/shurshick/wrtmonitor/releases/download/v0.13.1/wrtmonitor-openwrt-agent-v0.13.1.tar.gz
...
Команды `v0.13.1` выполняются только через авторизованный сервер и показываются в интерфейсах по реальным capabilities роутера:
