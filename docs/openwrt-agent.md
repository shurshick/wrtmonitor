...
Версия `0.13.6` использует capability schema 13, поддерживает `apk` и `opkg` и передаёт SSID, диапазон и интерфейс активных Wi-Fi-клиентов. После обновления сервера агент обновляется обычной кнопкой; переустановка не нужна.
...
Команды `v0.13.6` выполняются только через авторизованный сервер и показываются в интерфейсах по реальным capabilities роутера:
