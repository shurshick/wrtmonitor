...
Актуальный агент использует capability schema 18 и единый манифест обязательных runtime-зависимостей. Installer и обновление применяют этот манифест автоматически. Каждый payload-файл проверяется по SHA-256, а detached-манифест отдельно проверяется по встроенному Ed25519 public key.
