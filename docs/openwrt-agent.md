...
В `0.38.0` к каждому датчику добавлены только реально доступные `trip_point`, `temp*_max` и `temp*_crit`. Если ядро не предоставляет предел, сервер не придумывает рабочий диапазон. При наличии `thermal_pressure` агент также сообщает фактическое состояние троттлинга; на неподдерживаемой платформе поле имеет состояние `unsupported`.
...
Актуальный агент использует capability schema 20 и единый манифест обязательных runtime-зависимостей. Installer и обновление применяют этот манифест автоматически. Каждый payload-файл проверяется по SHA-256, а detached-манифест отдельно проверяется по встроенному Ed25519 public key.
...
- `nlbwmon`
- `ethtool`
- `tc-full`, `kmod-sched-core`, `kmod-sched-flower` и `kmod-sched-act-police` для индивидуальных лимитов клиентов и суммарных лимитов Wi-Fi-сетей
...
Лимиты скорости клиента применяются к фактическому MAC через `tc flower + police` на LAN bridge. Capability `clients.shaping` появляется только после подтверждения `tc` и всех kernel-модулей; если feed конкретной прошивки их не содержит, поля лимитов скрыты, а остальные политики клиента продолжают работать.
...
Wi-Fi capability публикуется только при наличии настоящего radio PHY. Оставшийся UCI-раздел `wireless` без физического модуля не считается поддержкой Wi-Fi и не включает пустые формы управления.

Capability `wifi.access_profile` публикуется только при наличии AP-интерфейса и исправного traffic control. Общий профиль основной или гостевой сети хранит снимок политики в UCI `wrtmonitor`, включает блокировку и недельное расписание конкретного SSID и применяет суммарные ограничения приёма/передачи к его runtime-интерфейсу. Агент повторно использует неизменившиеся фильтры и пишет UCI только при изменении наблюдаемого состояния.

QR для подключения создаётся без ручного ввода пароля: агент читает SSID, защиту и текущий ключ выбранной сети непосредственно из UCI. Wi-Fi URI отправляется серверу одноразово, не сохраняется в кэше агента, PostgreSQL или журнале операций и очищается после выдачи Web UI либо Android.
...
При этом `/etc/config/wrtmonitor` сохраняется, если отдельно не передан `--remove-config`. Параметры интервала и автообновления также сохраняются, но `server_url`, `device_id`, token и имя всегда заменяются подтверждённой сервером identity. При установке с `--token` installer сначала вызывает регистрацию агента и получает связанный с token `device_id`; пустой или старый идентификатор из UCI не используется.

До установки зависимостей и остановки службы installer проверяет, что системные каталоги доступны для записи. После загрузки payload отдельно проверяются место на диске и каталоги назначения. На read-only root или при нехватке места установка завершается, не изменяя рабочий агент.
...
6. проверяет запись в системные каталоги и достаточный запас места;
7. сохраняет backup текущего согласованного runtime;
8. собирает новое поколение библиотек в отдельном каталоге и проверяет его;
9. переключает активное поколение, init-скрипт и entrypoint, причём entrypoint заменяется последним;
10. запускает `ensure-dependencies` по манифесту новой версии;
11. при любой ошибке восстанавливает предыдущее поколение и entrypoint.

Поколения хранятся в `/usr/lib/wrtmonitor/releases/`. Маленький pointer-файл `current` переключается атомарным rename; entrypoint использует указанное поколение только когда его `agent-version.txt` совпадает с собственной версией. Поэтому прерванное обновление не может заставить старый entrypoint загрузить библиотеки новой версии.
