...
Разбор предупреждений CodeQL, ограничение CSP для терминала и проверка SSH-ключей стендов: [аудит 16.09.2026](security-review-2026-09-16.md).
...
- Web-терминал требует owner cookie, совпадающий Origin и capability конкретного роутера; UUID сессии дополнительно привязан к device token.
- Отдельный API произвольных shell-команд не поддерживается. Интерактивный shell доступен только владельцу в явно открытой Web-терминальной сессии.
...
- фоновый или публичный API произвольных shell-команд;
