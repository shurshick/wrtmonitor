...
| Wi-Fi 2.4/5/6 ГГц, SSID, guest, расписание и профили доступа | да | да | UCI, ubus, radio runtime и `tc` |
...
Полный контракт из 95 команд, риски, rollback и post-condition находятся в [command-matrix.md](command-matrix.md). Исключения поверхностей перечислены в `contracts/surface-exclusions.json` и проверяются CI.

Аппаратная применимость:

- Netis NX31: 91 команда поддержана, 4 не применимы;
- OpenWrt x86 без Wi-Fi: 76 команд поддержаны, 19 не применимы;
