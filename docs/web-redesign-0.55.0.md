...
Существующий поток `validate -> review -> apply -> verify` сохранён. Dialog больше не закрывается сразу после Apply: он показывает завершённую проверку и отправку команды до перехода к журналу операций. Выполнение на роутере, проверка результата и откат подтверждаются существующим механизмом команд. Для изменённых форм появляется общая панель несохранённых изменений с действиями Reset и Review/Apply. Успех не показывается до ответа существующего post-condition verifier. При обрыве связи во время проверки форма сообщает об ошибке.
...
- Локальный прогон: 341 backend/OpenWrt tests passed; 22 теста с дополнительными требованиями к окружению skipped. Полный набор с PostgreSQL и OpenWrt harness запускается в CI.
...
Новые маршруты страниц не добавлены: обновлены существующие обзор, Интернет, клиенты, Wi-Fi, сетевые правила, VPN, система, оборудование, обслуживание, терминал, события, список роутеров и аккаунт. Общие компоненты: оболочка, навигация, выбор роутера, статусы, формы, таблицы, диалог проверки и панель несохранённых изменений.

Старые определения корневой палитры заменены ссылками на токены. Остальные компонентные стили сохранены и перекрываются общим визуальным слоем; полного удаления legacy CSS этот релиз не заявляет. WebUI остаётся русскоязычным, как до редизайна. RU/EN Android не изменены. Доступность операций и состав полей определяются прежними API и capabilities; неподдерживаемые каталоги, сортировки и метрики не имитируются.
