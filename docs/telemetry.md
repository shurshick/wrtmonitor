...
`schema_version=2` остаётся форматом telemetry; capability report в `v0.10.0` имеет версию 10. Отсутствующие подсистемы возвращаются пустыми блоками и не ломают ingest. Блок `maintenance` содержит количество пакетов и обновлений, число cron-заданий, recovery mode и checksum подготовленной прошивки.
