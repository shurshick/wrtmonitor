...
- `WrtMonitorApi.kt`: 1003 → 471 строка;
- `SystemControlScreen.kt`: 714 → 582 строки;
- VPN-формы вынесены из `NetworkControlScreen.kt`;
- `app.css`: 867 → 220 строк, компоненты и адаптивность вынесены отдельно;
- новые пределы закреплены в `scripts/validate_architecture.py`.

Оставшиеся legacy trust/API и destructive hardware certification не смешивались с этим рефакторингом: они нужны для обновления уже установленных тестовых агентов и для отдельного физического прогона.
