...
execute_command() {
    ...
    case "$command_type" in
        ...
        firewall.set_forwarding)
            payload_file=/tmp/wrtmonitor-command-payload; printf '%s' "$command_payload" >"$payload_file"; forward_src="$(json_get_string "$payload_file" '@.src')"; forward_dest="$(json_get_string "$payload_file" '@.dest')"; forward_enabled="$(json_get_bool "$payload_file" '@.enabled')"; rm -f "$payload_file"; forward_ref="wrtmonitor_forward_${forward_src}_${forward_dest}"
            ...
            ;;
        firewall.set_rule)
            payload_file=/tmp/wrtmonitor-command-payload; printf '%s' "$command_payload" >"$payload_file"; rule_name="$(json_get_string "$payload_file" '@.name')"; rule_src="$(json_get_string "$payload_file" '@.src')"; rule_dest="$(json_get_string "$payload_file" '@.dest')"; rule_proto="$(json_get_string "$payload_file" '@.protocol')"; rule_src_ip="$(json_get_string "$payload_file" '@.src_ip')"; rule_dest_ip="$(json_get_string "$payload_file" '@.dest_ip')"; rule_src_port="$(json_get_string "$payload_file" '@.src_port')"; rule_dest_port="$(json_get_string "$payload_file" '@.dest_port')"; rule_target="$(json_get_string "$payload_file" '@.target')"; rm -f "$payload_file"; rule_ref="wrtmonitor_rule_$rule_name"; [ "$rule_proto" != tcpudp ] || rule_proto='tcp udp'; uci set "firewall.$rule_ref=rule"; uci set "firewall.$rule_ref.name=WrtMonitor-$rule_name"; uci set "firewall.$rule_ref.src=$rule_src"; for option in dest src_ip dest_ip src_port dest_port; do uci -q delete "firewall.$rule_ref.$option" || true; done; [ "$rule_dest" = '*' ] || uci set "firewall.$rule_ref.dest=$rule_dest"; uci set "firewall.$rule_ref.proto=$rule_proto"; uci set "firewall.$rule_ref.target=$rule_target"; [ -z "$rule_src_ip" ] || uci set "firewall.$rule_ref.src_ip=$rule_src_ip"; [ -z "$rule_dest_ip" ] || uci set "firewall.$rule_ref.dest_ip=$rule_dest_ip"; [ -z "$rule_src_port" ] || uci set "firewall.$rule_ref.src_port=$rule_src_port"; [ -z "$rule_dest_port" ] || uci set "firewall.$rule_ref.dest_port=$rule_dest_port"
        ...
        firewall.delete_rule)
            payload_file=/tmp/wrtmonitor-command-payload; printf '%s' "$command_payload" >"$payload_file"; rule_name="$(json_get_string "$payload_file" '@.name')"; rm -f "$payload_file"; if uci -q delete "firewall.wrtmonitor_rule_$rule_name" && uci commit firewall && /etc/init.d/firewall reload >/dev/null 2>&1; then result="$(command_success_result "firewall rule deleted")"; else status=failed; result="$(command_failed_result "rule not found")"; fi
            ;;
        system.set_hostname)
            printf '%s' "$command_payload" >/tmp/wrtmonitor-command-payload
