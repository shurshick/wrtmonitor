...
from ..models import ClientProfile, DeviceCommand, NetworkClient
from ..services.client_registry import client_response
...
def build_client_context(
    ...
    telemetry_clients: dict[str, Any],
) -> dict[str, Any]:
    ...
    online_count = sum(1 for item in clients if item.get("online"))
    recent_count = sum(1 for item in clients if item.get("presence_state") == "recent")
    lease_ipv4_by_mac = {
    ...
    return {
        ...
        "client_count": len(clients) if clients else telemetry_clients.get("count", 0),
        "online_client_count": online_count
        if clients
        else telemetry_clients.get("online_count", 0),
        "recent_client_count": recent_count
        if clients
...
def build_network_context(
    payload: dict[str, Any], network: dict[str, Any], dhcp_config: dict[str, Any]
) -> dict[str, Any]:
    ...
    return {
        ...
        "bridge_options": sorted(
            {str(item.get("name")) for item in bridges if item.get("name")}
        ),
...
def build_capability_context(agent: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "capabilities_groups": grouped_capabilities(capabilities, details),
...
def build_command_context(
    ...
    command_page: int,
    config: Settings,
    cleanup: Callable[..., int] = cleanup_device_command_history,
) -> dict[str, Any]:
    cleanup(
        db,
        device_id,
    ...
    page = min(max(command_page, 1), pages)
    commands = db.scalars(
    ...
    entries = [command_history_entry(command) for command in commands]
    support_entries = [command_history_entry(command) for command in support_commands]
    return {
        "commands": entries,
        "command_pagination": {
            "page": page,
        ...
        "download_artifacts": [
            ...
            and (
                command.result.get("archive_base64")
                or command.result.get("bundle_base64")
            )
        ],
        "latest_diagnostics": next(
            (
                command
                for command in support_entries
...
def build_system_view(
    telemetry_created_at: datetime | None,
    system: dict[str, Any],
    memory: dict[str, Any],
    cpu: dict[str, Any],
    ...
    age = (
        max(0, int((datetime.now(UTC) - telemetry_created_at).total_seconds()))
        if telemetry_created_at
        else None
    )
    ...
    return age, {
        ...
        "load_1m": load_1m,
        "cpu_cores": cpu_cores,
        "load_capacity_percent": load_percent,
        "load_level": (
            "низкая"
            if load_percent is not None and load_percent < 50
            else "умеренная"
